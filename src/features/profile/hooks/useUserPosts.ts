...
export function useUserPosts(authorId: string | number | undefined) {
  return useInfiniteQuery({
    queryKey: ["user", authorId, "posts"],
    queryFn: async ({ pageParam }) => {
      if (authorId == null) return []
      const res = await postsApi.getByAuthor(authorId as number, pageParam as number)
